...
public class FriendRequestAdapter extends ListAdapter<PendingFriendRequesterResponse, FriendRequestAdapter.ViewHolder> {
    ...
    public FriendRequestAdapter() {
        super(new DiffUtil.ItemCallback<PendingFriendRequesterResponse>() {
            @Override
            public boolean areItemsTheSame(@NonNull PendingFriendRequesterResponse oldItem, @NonNull PendingFriendRequesterResponse newItem) {
                return Objects.equals(oldItem.getUserId(), newItem.getUserId());
            }

            @Override
            public boolean areContentsTheSame(@NonNull PendingFriendRequesterResponse oldItem, @NonNull PendingFriendRequesterResponse newItem) {
    ...
    public interface OnActionClickListener {
        void onReject(@NonNull PendingFriendRequesterResponse request);
        void onAccept(@NonNull PendingFriendRequesterResponse request);
        void onSentAction(@NonNull PendingFriendRequesterResponse request);
    }
    ...
    public void submitRequests(List<PendingFriendRequesterResponse> requests) {
        submitList(requests);
    }

    public void setSendMode(ButtonType tabMode) {
    ...
    private int findPositionByUserId(@NonNull String userId) {
        List<PendingFriendRequesterResponse> currentList = getCurrentList();
    ...
    @NonNull
    private ButtonType resolveMode(@NonNull PendingFriendRequesterResponse request) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        PendingFriendRequesterResponse request = getItem(position);
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(PendingFriendRequesterResponse request, ButtonType isSendMode) {
            // Bind data
            binding.requestName.setText(request.getDisplayName());
            binding.requestMeta.setText(
                    TimeFormatter.formatDate(request.getCreatedAt())
            );
        ...
        void updateMode(ButtonType type) {
            if (type == ButtonType.SENT) {
                ...
                binding.btnSentAction.setText("Hủy");
                ...
                binding.incomingActions.setVisibility(View.GONE);
                binding.btnSentAction.setVisibility(View.VISIBLE);
                binding.btnSentAction.setText("Gửi lại");
            } else if (type == ButtonType.SEND) {
                binding.incomingActions.setVisibility(View.GONE);
                binding.btnSentAction.setVisibility(View.VISIBLE);
                binding.btnSentAction.setText("Gửi");
