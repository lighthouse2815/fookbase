...
import com.dangngulon.frontend.feature.zola.presentation.ui.adapters.ContactAdapter;
import com.dangngulon.frontend.feature.zola.presentation.viewmodel.ContactViewModel;
...
public class ContactsFragment extends Fragment {
    ...
    private void setupRecyclerView() {
        ...
        adapter.setOnContactClickListener(contact -> {
            UiHelper.showToast(requireContext(), "Chat voi: " + contact.getNickName());
        });
    }

    private void setupClickListeners() {
        NavController navController = NavHostFragment.findNavController(this);
        binding.addFriendAction.setOnClickListener(v -> navController.navigate(R.id.action_global_addFriend));
    }

    private void observeViewModel() {
        contactViewModel.getContactListResult().observe(getViewLifecycleOwner(), result -> {
            if (result == null) {
                return;
            }
            ...
            switch (result.getStatus()) {
                ...
                case SUCCESS:
                    showLoading(false);
                    List<ContactUiModel> list = result.getData();
