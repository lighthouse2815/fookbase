...
public sealed class GameRoomService : IGameRoomService
{
    ...
    public GameRoomDto JoinRoomByCode(string roomCode, GamePlayerIdentityDto player, string? connectionId = null)
    {
        if (string.IsNullOrWhiteSpace(roomCode))
        {
            throw new BusinessException(ErrorCode.VALIDATION_ERROR, "roomCode is required.");
        }
    ...
    public GameRoomDto MarkRoomPlaying(Guid roomId, Guid hostUserId)
    {
        lock (_syncRoot)
        {
            ...
            {
                throw new BusinessException(ErrorCode.FORBIDDEN, "Only host can start the game.");
            }

            if (room.Players.Count < room.MaxPlayers)
            {
                throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "Not enough players to start.");
            }
    ...
    public GameRoomDto RegisterRematchVote(Guid roomId, Guid userId, out bool allPlayersReady)
    {
        lock (_syncRoot)
        {
            ...
            {
                throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "Rematch can only be requested after game over.");
            }

            if (room.Players.All(item => item.UserId != userId))
            {
                throw new BusinessException(ErrorCode.FORBIDDEN, "Player is not in room.");
            }
    ...
    private GameRoomDto JoinRoomCore(Guid roomId, GamePlayerIdentityDto player, string? connectionId)
    {
        ...
        {
            throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "Room is already playing.");
        }

        if (room.Players.Count >= room.MaxPlayers)
        {
            throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "Room is full.");
        }
    ...
    private string GenerateUniqueRoomCode()
    {
        ...
        throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "Cannot allocate room code.");
    }
