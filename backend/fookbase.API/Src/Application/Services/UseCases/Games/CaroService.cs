...
public sealed class CaroService : ICaroService
{
    ...
    public CaroStateDto StartGame(GameRoomDto room, int boardSize = 15)
    {
        ...
        {
            throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "Caro requires 2 players.");
        }

        if (boardSize < 10 || boardSize > 30)
        {
            throw new BusinessException(ErrorCode.VALIDATION_ERROR, "Board size must be between 10 and 30.");
        }
