...
public class HashtagService : IHashtagService
{
    ...
    public async Task<HashtagResponseDto> CreateAsync(CreateHashtagRequestDto request, CancellationToken cancellationToken)
    {
        ...
        var existing = await _hashtagRepository.GetByNormalizedNameAsync(normalizedName, cancellationToken);
        if (existing is not null)
        {
            throw new ArgumentException("Hashtag already exists.");
        }
    ...
    public async Task<HashtagResponseDto> UpdateAsync(Guid hashtagId, UpdateHashtagRequestDto request, CancellationToken cancellationToken)
    {
        ...
        var duplicate = await _hashtagRepository.GetByNormalizedNameAsync(normalizedName, cancellationToken);
        ...
        hashtag.NormalizedName = normalizedName;
    ...
    public async Task DeleteAsync(Guid hashtagId, CancellationToken cancellationToken)
    {
        ...
        if (usageCount > 0)
        {
            throw new ArgumentException("Cannot delete hashtag because it is used by posts.");
        }
    ...
    private static string NormalizeName(string value)
    {
        ...
        {
            throw new ArgumentException("Hashtag name is required.");
        }
