...
export const FriendSearchPage = () => {
  ...
  useEffect(() => {
    ...
    const loadResults = async () => {
      ...
      try {
        const response = await profileService.searchProfiles(keywordQuery);
        if (isCancelled) {
          return;
        }
    ...
    <div className="space-y-4">
      <header className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900/75 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">Tim ban be</h1>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Nhap so dien thoai hoac ten hien thi va nhan Enter de tim nhanh.
            </p>
          </div>
        ...
        <form onSubmit={handleSearchSubmit} className="mt-4">
          <label className="relative block">
            ...
            <input
              ...
              placeholder="Nhap so dien thoai hoac ten hien thi..."
