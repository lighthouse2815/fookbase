...
import type { FriendFilter, FriendsPageFetchState, FriendsTab } from '@/features/friendship/types/pages';
import { useFriendsDerivedData } from '@/features/friendship/hooks/useFriendsDerivedData';
import {
  parseFriendsTab,
  sanitizeFriends,
  sanitizeRequests,
  ...
  toSentRequest,
  toSuggestion,
} from '@/features/friendship/utils/page.util';

const EMPTY_REQUESTS: FriendRequest[] = [];
const EMPTY_FRIENDS: FriendUser[] = [];
const EMPTY_SUGGESTIONS: FriendSuggestion[] = [];
...
export const useFriendsPage = () => {
  ...
  const loadFriendData = useCallback(async (options?: { silent?: boolean }) => {
    ...
    setSuggestions(
      syncPresenceByUserId(sanitizeSuggestions(resolvedSuggestions, EMPTY_SUGGESTIONS), latestPresenceByUserId),
    );

    setFriends(
      syncPresenceByUserId(
        sanitizeFriends(friendsResult.status === 'fulfilled' ? friendsResult.value : undefined, EMPTY_FRIENDS),
        latestPresenceByUserId,
      ),
    );
...
export type UseFriendsPageReturn = ReturnType<typeof useFriendsPage>;
