...
export const AppReviewForm = ({
  ...
  const resolvedSubmitLabel = submitLabel ?? tx('Gửi đánh giá', 'Submit review');
  ...
  return (
    ...
    >
      ...
      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">
        {tx('Đánh giá sao', 'Rating')}
        <Controller
          control={control}
          name="rating"
          rules={{
            required: tx('Vui lòng chọn số sao.', 'Please select a rating.'),
            min: { value: 1, message: tx('Đánh giá tối thiểu 1 sao.', 'Minimum rating is 1 star.') },
            max: { value: 5, message: tx('Đánh giá tối đa 5 sao.', 'Maximum rating is 5 stars.') },
          }}
          render={({ field }) => (
            <StarRatingInput
              value={field.value}
      ...
      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">
        ...
        <input
          {...register('displayName', {
            required: tx('Vui lòng nhập tên hiển thị.', 'Display name is required.'),
            minLength: { value: 2, message: tx('Tên tối thiểu 2 ký tự.', 'Display name must be at least 2 characters.') },
            maxLength: { value: 80, message: tx('Tên tối đa 80 ký tự.', 'Display name must be at most 80 characters.') },
          })}
          placeholder={tx('Nhập tên hiển thị công khai', 'Public display name')}
          disabled={isSubmitting || isDeleting}
      ...
      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">
        ...
        <textarea
          {...register('comment', {
            required: tx('Vui lòng nhập nhận xét.', 'Comment is required.'),
            minLength: { value: 3, message: tx('Nhận xét tối thiểu 3 ký tự.', 'Comment must be at least 3 characters.') },
            maxLength: { value: 1000, message: tx('Nhận xét tối đa 1000 ký tự.', 'Comment must be at most 1000 characters.') },
          })}
          rows={4}
          placeholder={tx('Trải nghiệm của bạn với ứng dụng...', 'Share your experience with this app...')}
          disabled={isSubmitting || isDeleting}
          className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none transition focus:border-brand-500 disabled:cursor-not-allowed disabled:opacity-70 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
        />
      ...
      <div className="flex flex-wrap justify-end gap-2">
        {onDelete ? (
          ...
          >
            {isDeleting ? tx('Đang xóa...', 'Deleting...') : tx('Xóa đánh giá', 'Delete review')}
          </button>
        ) : null}
        <button
          type="submit"
        ...
        >
          {isSubmitting ? tx('Đang gửi...', 'Submitting...') : resolvedSubmitLabel}
        </button>
      </div>
    </form>
  );
