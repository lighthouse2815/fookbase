  ...
  return (
    ...
    >
      <AuthCard
        tone={tone}
        title={isOtpStep ? t('auth.verifyEmailTitle') : t('auth.registerTitle')}
        subtitle={isOtpStep ? t('auth.verifyEmailSubtitle') : t('auth.registerSubtitle')}
        headerKey={`register-header-${step}`}
      ...
      >
        <AuthFormTransition transitionKey={`register-content-${step}`}>
          {isOtpStep ? (
            <form
              className="space-y-4"
